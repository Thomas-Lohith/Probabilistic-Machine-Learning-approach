...
from model_vae import create_vae_model, vae_loss_function
from utils import set_seed


class VAETrainer:
    """
    Trainer for LSTM Variational Autoencoder
    
    Key differences from standard autoencoder training:
    1. Loss function: MSE + KL Divergence
    2. Model outputs: (reconstructed, mu, log_var) instead of just reconstructed
    3. KL weight annealing (optional): gradually increase KL weight during training
    """
    ...
    def __init__(self, 
        ...
        Args:
            ...
            kl_annealing: Whether to anneal KL weight (start from 0, increase to kl_weight)
            kl_annealing_epochs: Number of epochs to anneal KL weight over
        """
        self.model = model
    ...
    def update_kl_weight(self, epoch: int):
        ...
        KL annealing helps VAE training by gradually introducing the KL constraint.
    ...
    def train(self, epochs: int = None, checkpoint_dir: Path = None) -> Dict:
        """
        Train VAE for specified number of epochs.
...
def train_vae_phase1(data_path: str = None,
                     ...
                     kl_annealing_epochs: int = 10):
    """
    Train VAE on single day (Phase 1).
    
    Args:
        data_path: Path to preprocessed data
        checkpoint_dir: Directory to save checkpoints
        epochs: Number of epochs
        quick_test: Whether to run quick test (fewer epochs)
